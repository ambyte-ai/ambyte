...
export function formatIsoDuration(duration: string): string {
	...
	const [_, years, months, weeks, days, hours, minutes, seconds] = matches;
	...
	if (minutes)
		parts.push(`${minutes} Minute${parseInt(minutes) > 1 ? "s" : ""}`);
	if (seconds)
		parts.push(`${seconds} Second${parseInt(seconds) > 1 ? "s" : ""}`);

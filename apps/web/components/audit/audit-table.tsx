...
function DecisionBadge({ decision }: { decision: Decision }) {
	...
	if (decision === "DRY_RUN_DENY") {
		return (
			<Badge
				variant="outline"
				className="border-amber-500/50 border-dashed text-amber-500 bg-amber-500/10 font-semibold rounded px-2"
			>
				DRY RUN DENY
			</Badge>
		);
	...
	return (
		<Badge variant="outline" className="rounded px-2">
			{decision}
		</Badge>
	);
}
					...
					<Table>
						<TableHeader className="bg-muted/20 sticky top-0 z-10 backdrop-blur-sm">
							<TableRow className="hover:bg-transparent border-b border-border/50">
								...
								<TableHead className="w-[180px] text-xs uppercase tracking-wider">
									Time
								</TableHead>
						...
						<TableBody>
							{logs.length === 0 && !isLoading ? (
								...
								logs.map((log) => {
									...
									const reason =
										log.reason_trace?.decision_reason ||
									...
									return (
										...
										>
											...
											<TableCell>
												<span
													className="font-mono text-xs text-foreground/90 truncate block max-w-[180px]"
													title={log.actor_id}
												>
													{log.actor_id}
												</span>

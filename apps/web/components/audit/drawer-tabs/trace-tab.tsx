				...
				<div className="flex items-center gap-3 mb-3">
					...
					<div>
						...
						<p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
							Action: <span className="text-foreground/80">{log.action}</span>{" "}
							on{" "}
		...
		return (
			<div className="space-y-3">
				...
				<p className="text-xs text-muted-foreground italic">
					No specific policy contributions recorded for this evaluation.
				</p>
			</div>
		);
	}
			...
			<div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 space-y-2">
				<div className="flex items-start gap-2">
					...
					<p className="text-xs text-amber-200/80 leading-relaxed">
						This resource inherits restrictions from upstream data sources via
						lineage propagation. The following constraints &quot;poison&quot;
						downstream usage:
					</p>
				</div>
...
function CacheBadge({ log }: { log: AuditLog }) {
	...
	return (
		<div className="flex items-center gap-2">
			<span className="text-xs text-muted-foreground">Engine Response:</span>
			<Badge
				variant="outline"
				className={cn(
			...
			{log.reason_trace?.resolved_policy_hash && (
				<span
					className="text-[10px] font-mono text-muted-foreground/50 truncate max-w-[120px]"
					title={log.reason_trace.resolved_policy_hash}
				>

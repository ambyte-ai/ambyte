...
type VerifyStep = "idle" | "fetching" | "recomputing" | "verifying" | "verified" | "failed";
        ...
        >
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <Icon
                        className={cn("h-3.5 w-3.5", iconColors[accent])}
                    />
            ...
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-6 text-center space-y-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-amber-200/60 mt-1.5 max-w-sm mx-auto leading-relaxed">
                        This log entry is currently in the high-speed memory
                        buffer. It will be cryptographically sealed into an
                        immutable block once the buffer threshold is reached.
                    </p>
                </div>
                <Badge
                    variant="outline"
                ...
                <div className="mt-4 rounded-xl border-2 border-emerald-500/30 bg-emerald-500/5 p-6 text-center space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-emerald-300/60 mt-1">
                            Signature &amp; Merkle path match the sealed block
                            root.
                        </p>
                    </div>
                ...
                <div className="rounded-xl border border-emerald-500/20 bg-card/50 overflow-hidden">
                    ...
                    <div className="p-3 space-y-1">
                        ...
                        <p className="text-xs font-mono text-foreground/80">
                            {new Date(
                                proof.block_header.timestamp_start
                            ).toLocaleString()}{" "}
                            →{" "}
                            {new Date(
                                proof.block_header.timestamp_end
                            ).toLocaleString()}
                        </p>
                    </div>
                </div>
...
export function IntegrityTab({ log }: IntegrityTabProps) {
    ...
    const { proof, isLoading, isError } = useAuditProof(
        isSealed ? log.id : null
    );

...
import { type GraphNode, useLineageAnalysis } from "@/hooks/use-lineage";
...
export function LineageInspector() {
	...
	const { analysis, isLoading: isAnalyzing } = useLineageAnalysis(
		activeNode ? selectedNodeId : null,
	);
	...
	if (!selectedNodeId) {
		return (
			<Sheet open={false} onOpenChange={handleClose}>
					...
					<div className="p-6 pb-4 bg-muted/10 border-b border-border/50">
						<SheetHeader>
							<div className="flex items-center gap-2 mb-2">
								<Badge
									variant="outline"
									className="text-[10px] font-mono tracking-wider bg-background"
								>
									Execution Run
								</Badge>
								{isSuccess ? (
					...
					<div className="p-6 pb-0 bg-muted/10 shrink-0">
						...
						<Tabs defaultValue="governance" className="w-full">
							...
							<ScrollArea className="h-[calc(100vh-140px)]">
								...
								<TabsContent
									value="governance"
									className="m-0 p-6 space-y-8 outline-none"
								>
									...
									<div className="space-y-3">
										...
										<div className="grid grid-cols-2 gap-4">
											...
											<div className="p-4 rounded-xl border bg-card shadow-sm flex flex-col gap-2">
												<span className="text-[10px] uppercase text-muted-foreground font-semibold flex items-center gap-1">
													<GitBranch className="w-3 h-3" /> Max Inherited
													Sensitivity
												</span>
												{isAnalyzing ? (
													...
													<SensitivityBadge
														level={analysis?.inherited_sensitivity}
													/>
												)}
											</div>
										</div>
										<p className="text-[11px] text-muted-foreground leading-relaxed mt-2">
											Risk and sensitivity escalate upwards. If this asset
											consumes from a Highly Restricted source, it inherits that
										...
										{isAnalyzing ? (
											...
											<div className="space-y-4">
												...
												<div className="p-4 rounded-xl border border-rose-500/30 bg-rose-500/5 shadow-sm space-y-3">
													<div className="flex items-start gap-3">
														...
														<div>
															<h4 className="text-sm font-semibold text-rose-500">
																Usage Restricted by Ancestor
															</h4>
															<p className="text-xs text-rose-500/80 leading-relaxed mt-1">
																This asset contains data derived from sources
																that explicitly forbid secondary usage (e.g., AI
													...
													<div className="pt-3 border-t border-rose-500/20">
														<span className="text-[10px] font-semibold uppercase text-rose-500/70 block mb-2">
															Tainted Sources:
														</span>
														<ul className="space-y-1.5">
															{analysis.poisoned_constraints.map((c) => (
																<li
																	key={c}
																	className="text-xs font-mono text-rose-400 bg-rose-950/30 px-2 py-1 rounded border border-rose-500/20 truncate"
																>
																	{c}
																</li>
															))}
												...
												>
													{isTraced
														? "Trace Currently Active on Canvas"
														: "Visually Trace Blocker in Graph"}
													{!isTraced && (
											...
											<div className="p-6 text-center rounded-xl border border-dashed bg-emerald-500/5">
												...
												<p className="text-xs text-muted-foreground mt-1 max-w-[250px] mx-auto">
													This asset has not inherited any restrictive usage
													policies from its data sources.
												</p>
											</div>
										)}
									</div>
								...
								<TabsContent
									value="overview"
									className="m-0 p-6 space-y-6 outline-none"
								>
									<div className="space-y-3">
										<h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
											Resource Tags

...
const PlatformIcon = ({ platform, className }: { platform: string; className?: string }) => {
    ...
    if (p.includes("snowflake")) return <Database className={cn("text-cyan-500", className)} />;
    if (p.includes("databricks")) return <Server className={cn("text-orange-500", className)} />;
    if (p.includes("s3") || p.includes("aws")) return <Cloud className={cn("text-yellow-500", className)} />;
    if (p.includes("local") || p.includes("file")) return <FileText className={cn("text-zinc-500", className)} />;
...
export function ResourceNode({ data, id }: { data: GraphNode; id: string }) {
    ...
    const borderClass = sensitivityColors[data.sensitivity?.toUpperCase()] || sensitivityColors.UNSPECIFIED;
                ...
                <div className="flex items-center justify-between mt-1">
                    ...
                    <Badge
                        ...
                        className={cn(
                            ...
                            data.sensitivity === "RESTRICTED" && "text-rose-500 border-rose-500/20 bg-rose-500/5",
                            data.sensitivity === "CONFIDENTIAL" && "text-amber-500 border-amber-500/20 bg-amber-500/5"
                        )}
                    >
                        {data.sensitivity}
                    </Badge>

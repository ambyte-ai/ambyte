...
export function AuditConfig() {
	...
	return (
		<div className="space-y-6 animate-in fade-in duration-500">
			{/* Header Section */}
			<div>
				<h3 className="text-lg font-medium text-foreground">Audit Integrity</h3>
				<p className="text-sm text-muted-foreground">
					Cryptographic keys used to secure and verify your policy enforcement
					ledger.
				</p>
			</div>
			...
			<Card className="border-border/50 shadow-sm bg-card/50 overflow-hidden">
				<CardHeader>
					...
					<CardDescription className="leading-relaxed max-w-2xl">
						Ambyte seals audit logs into immutable blocks using an Ed25519
						private key. Use the public key below to mathematically verify the
						integrity of your logs via the CLI.
					</CardDescription>
				</CardHeader>
					...
					{isLoading ? (
						...
						<>
							...
							<div className="rounded-xl border border-zinc-800 bg-[#0D0D0D] shadow-2xl overflow-hidden ring-1 ring-white/5">
								...
								<div className="p-4 space-y-1.5 font-mono text-[11px] leading-relaxed">
									...
									<div className="flex gap-1">
										...
										<span className="text-amber-400/70">&lt;log-id&gt;</span>
									</div>
								</div>
							</div>
							...
							<p className="text-[12px] text-muted-foreground/70 leading-relaxed">
								This key is derived from the Ambyte system signing key and
								cannot be changed. It is safe to share — it can only{" "}
								<em>verify</em> signatures, not create them.
							</p>
						</>
					)}
				</CardContent>

...
import { EnforcementLevel, type Obligation } from "@/types/obligation";
...
export function useObligations(filters?: UseObligationsFilters) {
	...
	if (
		filters?.enforcement_level !== undefined &&
		filters.enforcement_level !== EnforcementLevel.UNSPECIFIED
	) {
...
export function useObligation(slug?: string | null) {
	...
	const { data, error, isLoading, mutate } = useSWR<Obligation>(key, fetcher, {
		revalidateOnFocus: false,
	});
